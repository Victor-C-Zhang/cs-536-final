...
from utils import AverageMeter, cosine_distance, get_variable
...
def main():
    ...
    val_loader=get_loader.get_val_loader(opts.batch_size)
    ...
    wordEmb_params = [kv[1] for kv in model.named_parameters() if
                      'textMLP.mBERT.embeddings' in kv[0] or 'word_embeddings' in kv[0]]
    ...
    optimizer = torch.optim.Adam([
        {'params': heads_params},
        # these correspond to the text and vision "heads" (layer before foodSpace and after resnet and text embedder)
        {'params': vision_params, 'lr': opts.lr * opts.freeVision, 'weight_decay': opts.weight_decay},
        # resnet embeddings
        {'params': wordEmb_params, 'lr': opts.lr * opts.freeWordEmb, 'weight_decay': opts.weight_decay},
        # word embeddings
        {'params': text_params, 'lr': opts.lr * opts.freeText, 'weight_decay': opts.weight_decay}
        # text embedder params except word embeddings
    ...
    train_loader = get_loader.get_train_loader(opts.batch_size)
    for epoch in range(opts.start_epoch, opts.epochs):
        ...
        iter = train(train_loader, None, model, optimizer, epoch, iter, train_writer, train_writer_text)
        ...
        if (epoch) % opts.valfreq == 0:
            ...
            if curr_val < best_val:
                if glob.glob(opts.snapshots + 'model_BEST_VAL*'): os.remove(
                    glob.glob(opts.snapshots + 'model_BEST_VAL*')[0])
            ...
            if cum_recall > best_rec:
                if glob.glob(opts.snapshots + 'model_BEST_REC*'): os.remove(
                    glob.glob(opts.snapshots + 'model_BEST_REC*')[0])
                filename = opts.snapshots + 'model_BEST_REC_e%03d_v-%.3f_cr-%.4f.pth.tar' % (
                    state['epoch'], state['curr_val'], cum_recall)
                torch.save(state, filename)
            if epoch in opts.save_epoch:
                filename=opts.snapshots+f'model_{epoch}.pth.tar'
                torch.save(state, filename)
                print("save model")
